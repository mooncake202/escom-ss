...
export default function RevisarSolicitudOferta() {
  ...
  useEffect(() => {
    apiFetch("/ofertas/pendientes")
      .then(setPendientes)
      .catch((err) => setErrorCarga(err.message || "No se pudieron cargar las solicitudes."))
  ...
  function seleccionar(s) {
    ...
    setMotivos("");
    setErrores({});
    if (s.id === destacadoId) setDestacadoId(null);
  }
  ...
  async function confirmarAprobacion() {
    try {
      await apiFetch(`/ofertas/${seleccionada.id}/decidir`, {
        method: "POST",
        body: JSON.stringify({ decision: "aprobar" }),
      });
      const esIndividual = seleccionada.modalidad === "individual";
      const resuelta = { ...seleccionada, decision: "aprobado", cuposAutorizados: seleccionada.cuposSolicitados };
      setResueltas(prev => [...prev, resuelta]);
      mostrarToast(
        esIndividual
          ? "Oferta individual aprobada · Cupo: 1. El profesor ha sido notificado."
          : `Proyecto aprobado · ${seleccionada.cuposSolicitados} cupo(s). El profesor ha sido notificado.`
      );
  ...
  async function confirmarRechazo() {
    ...
    try {
      await apiFetch(`/ofertas/${seleccionada.id}/decidir`, {
        method: "POST",
        body: JSON.stringify({ decision: "rechazar", motivoRechazo: motivos.trim() }),
      });
      ...
      <div style={{ maxWidth: 980, margin: "0 auto", width: "100%" }}>
        ...
        {cargando ? (
          <div style={{
            ...
            <p style={{ margin: 0, fontSize: 14, color: C.textMuted }}>Cargando solicitudes...</p>
          </div>
        ) : (
                ...
                {pendientes.length === 0 ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column" }}>
                    {pendientes.map(s => {
                      ...
                      const destacado = s.id === destacadoId;
                      return (
                        <button
                          key={s.id}
                          ...
                          style={{
                            ...
                            border: `2px solid ${activa ? C.accent : destacado ? C.info : "transparent"}`,
                        ...
                        >
                          ...
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                            <span style={{ fontSize: 11, color: C.textDisabled }}>{formatearFecha(s.fechaSolicitud)}</span>
                            <span style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              {!seleccionada ? (
                ...
                <div style={{
                    ...
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "1rem", flexWrap: "wrap" }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <p style={{ margin: "0 0 6px", fontSize: 13, color: C.textMuted }}>
                          {seleccionada.profesor} · Solicitado: {formatearFecha(seleccionada.fechaSolicitud)}
                        </p>
                      </div>
                  ...
                  <Campo label="Programa SISS" C={C}>
                    <p style={{ margin: 0, fontSize: 13, color: C.textPrimary, lineHeight: 1.6, fontWeight: 500 }}>
                      {seleccionada.programaSISS}
                    </p>
                  </Campo>

                  <Campo label="Actividad SISS" C={C}>
                    <p style={{ margin: 0, fontSize: 13, color: C.textPrimary, lineHeight: 1.6, fontWeight: 500 }}>
                      {seleccionada.tituloSISS}
                    </p>
                  </Campo>

                  <Campo label="Descripción y actividades a realizar" C={C}>
